...
public class JobBrowserController {
    ...
    private Parent buildFilters() {
        ...
        // 岗位状态 + 申请结果（ACCEPTED/REJECTED 为独立项，与 OPEN/CLOSED 等平级）
    ...
    /**
     * Maps Job Type combo display string to {@link JobType}; {@code null} means no type filter.
     */
    ...
    private void loadJobs() {
        ...
        // TA 浏览时不展示草稿；Browse 页状态下拉不含 DRAFT（草稿在 MO 管理页处理）
        if (user.getRole() == Role.TA) {
            jobs = jobs.stream()
        ...
        // 按「已录取 / 已拒绝」筛选（仅与当前登录 TA 的申请记录相关）
        ...
        // TA：CLOSED / EXPIRED 与列表角标一致（避免选 CLOSED 仍出现 Accepted 等）
        // OPEN 不按角标过滤：仍在招聘中的岗位含 Applied/Under Review 等，应一并出现在「OPEN」下
    ...
    /**
     * 与职位卡片右上角角标一致（有申请时优先显示申请状态），供 TA 筛选与列表展示共用。
     */
